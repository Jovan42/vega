...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public EmployeeOutputDto add(EmployeeInputDto employeeInputDto) {
        ...
        return mapper.employeeToEmployeeOutputDto(
                employeeRepository.save(mapper.employeeDtoToEmployee(employeeInputDto)));
    }
    ...
    public EmployeeOutputDto update(EmployeeInputDto employeeInputDto) {
        String oldPassword =
                employeeRepository
                        .findById(employeeInputDto.getId())
                        .orElseThrow(
                                () ->
                                        new NotFoundException(
                                                "Team member with id '"
                                                        + employeeInputDto.getId()
                                                        + "' is not found"))
                        .getPassword();
    ...
    public EmployeeListDto getAllWithoutProject() {
        return new EmployeeListDto(
                employeeRepository.findAllByProjectMember_IdIsNull().stream()
                        .map(mapper::employeeToEmployeeOutputDto)
                        .collect(Collectors.toList()));
    }
    ...
    public EmployeeListDto getAllWithoutTeam() {
        return new EmployeeListDto(
                employeeRepository.findAllByTeamsIsNull().stream()
                        .map(mapper::employeeToEmployeeOutputDto)
                        .collect(Collectors.toList()));
    }
}
